...
create_military_MER <- function(mer_df, VAL_YEAR, VAL_QUARTER, MILITARY_PSNU_PATH){
    ...
    TX_CURR_military <- temp_military %>%
        ...
        mutate(TX_CURR_new = round(TX_CURR * percent_total,0),
               TX_PVLS_D_new = round(TX_PVLS_D * percent_total,0),
               TX_PVLS_N_new = round(pvls_percent * TX_PVLS_D_new,0)
        )%>%
        select(-c(TX_PVLS_D, TX_PVLS_N, pvls_percent, col_join, percent_total,
                  TX_CURR)) %>%
        rename(TX_CURR = TX_CURR_new,
               TX_PVLS_N = TX_PVLS_N_new,
               TX_PVLS_D = TX_PVLS_D_new) %>%
        pivot_longer(cols = starts_with("TX"),
                     names_to = "indicator",
                     values_to = "value") %>%
        mutate(standarddizeddisaggregate = "military") %>%
