...
# Spectrum Dataset
 spectrum <- spectrum_df %>%
  ...
  mutate(name = case_when(is.na(MER_name) ~ area_name,
                          TRUE ~ MER_name),
         year = year(my(quarter_label)),
         sex = str_to_title(sex),
         age_group_label = case_when(age_group_label == "<1"~"<01",
                                     age_group_label == "1-4"~"01-04",
                                     age_group_label == "5-9" ~ "05-09",
                                     TRUE ~ age_group_label),
         age_group_type = case_when(age_group_label %in% c("<01","01-04", "05-09","10-14", "<15") ~ "ped",
                                                           TRUE ~ "adult")
         ) %>%
  ...
  filter( sex != "Both",
          ...
          area_level == 3,
          year == VAL_YEAR,
          age_group_label %in% c("<01","01-04","05-09","10-14","15-19","20-24","25-29","30-34","35-39","40-44","45-49",
                                 "50-54","55-59","60-64","65+")
  ) %>%
  select(-c(MER_name, area_name, quarter_label, calendar_quarter, median, mode, area_id, area_level_label, age_group,
            indicator, name, area_level, year, se, upper, lower)) %>%
  rename(value = mean,
    ...
    #MER Dataset
    mer <- mer_df %>%
    mutate(psnuuid = case_when(psnu == "Chonguene" ~ "jDWCkBXYllV",
                               psnu == "Mandlakaze" ~ "VjJPal9jqUi",
                               TRUE ~ psnuuid),
           age_group_type = case_when(ageasentered %in% c("<01", "01-04", "05-09", "10-14", "<15") ~ "ped",
                                     TRUE ~ "adult")
           )

# CREATE EPI MODEL DATA ---------------------------------------------------

TX_CURR   <- mozR::create_data(mer,
                         "TX_CURR",
                         c("Age/Sex/HIVStatus", "Age Aggregated/Sex/HIVStatus"),
                         num_dem = "N",
                         label = "TX_CURR")

TX_PVLS_N <- mozR::create_data(mer,
                         "TX_PVLS",
                         c("Age/Sex/Indication/HIVStatus", "Age Aggregated/Sex/Indication/HIVStatus"),
                         num_dem = "N",
                         label = "TX_PVLS_N")

TX_PVLS_D <- mozR::create_data(mer,
                          "TX_PVLS",
                          c("Age/Sex/Indication/HIVStatus", "Age Aggregated/Sex/Indication/HIVStatus"),
                          num_dem = "D",
                          label = "TX_PVLS_D")
...
epi <- spectrum %>%
    ...
    pivot_wider(names_from = "indicator", values_from = "value", values_fn = list(values = sum)) %>%
    clean_names()  %>%
    mutate(tx_curr_adj = case_when(  (tx_curr >= number_plhiv_aware) ~ number_plhiv_aware,
                                      TRUE ~ tx_curr
                                  ),
           period      = paste(VAL_YEAR, VAL_QUARTER)
           ) %>%

   pivot_longer(cols = c(tx_curr, tx_curr_adj, plhiv, number_plhiv_aware, tx_pvls_n, tx_pvls_d),
                names_to = "indicator",
                values_to = "value")


# OUTPUT ----------------------------------------------------------------------------
write_csv2(epi, "Dataout/epi.csv")
